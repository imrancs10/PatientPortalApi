...
namespace PatientPortal.Mobile.Web.Services
{
    public class UserService : IUserService
    {
        ...
        public UserService(IDatabase swn402Db, IPasswordHasherService passwordHasherService
            , Func<PatientPortalMobileContext> dbContextProvider, IContactsService contactsService)
        {
            _swn402Db = swn402Db;
        ...
        private async Task<bool> IsExistingRecipient(long recipientId, PatientPortalMobileContext dbContext)
        {
            return await dbContext.MobileUser
                            .AnyAsync(x => x.RecipientId == recipientId && !x.ENSUserId.HasValue);
        }

        private async Task<bool> IsExistingEmail(string email, PatientPortalMobileContext dbContext)
        {
            return await dbContext.MobileUser
