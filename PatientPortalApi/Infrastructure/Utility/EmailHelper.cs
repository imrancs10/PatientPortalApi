...
namespace PatientPortalApi.Infrastructure.Utility
{
    public class EmailHelper
    {
        ...
        public static string GetAppointmentSuccessEmail(string firstname, string middlename, string lastname, 
            string doctorname, DateTime apptime, string deptname,bool IsActiveAppointmentMessage,string AppointmentMessage)
        {
            string body = string.Format("Hi {0} {1} {2}<br/><br/>", firstname, middlename, lastname);
            ...
            if (IsActiveAppointmentMessage && !string.IsNullOrEmpty(AppointmentMessage))
            {
                body += "<br/>" + string.Format("Note : {0} <br/>", AppointmentMessage) + "<br/><br/>";
            }
            body += "Thank You,<br/>";
            body += "Patient Portal Information System Customer Support";
            return body;
